...
| `knowledge/fetch-gotchas.md` | 以 WebFetch 抓內文或查核發布日期時 | iThome、openai.com、inside.com.tw 常回 403；technews.tw 會轉址到 cdn 網域且日期歸檔頁不全；blog.cloudflare.com 與 anthropic.com 的 slug 不可由標題推測；全球中央會 301 轉址且 URL 編號不等於發布日；活動預告的實施日不等於發布日；標題與內文矛盾的來源直接淘汰；WebFetch 摘要會改寫時間詞與補出日期，日期與數字改用 curl 取 JSON-LD 核對。 |
