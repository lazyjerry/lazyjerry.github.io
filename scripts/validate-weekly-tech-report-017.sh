...
DELIVERY="$ROOT/tech/2026-08-22"
REPORT="$TASK/report.md"
SOURCE="$TASK/source.md"
SKILL_SCRIPTS="/Users/lazyjerry/.claude/skills/tech-report-research/scripts"
[ -d "$SKILL_SCRIPTS" ] || SKILL_SCRIPTS="/Users/lazyjerry/.ai-global/skills/tech-report-research/scripts"
...
sh "$SKILL_SCRIPTS/validate-task-completion.sh" "$ROOT" weekly-tech-report-017

# 交付物完整性
[ -s "$DELIVERY/2026-08-22 科技服務資訊週記.md" ] || fail "缺少交付報告"
for f in source.md task_plan.md note.md references.md; do
  [ -s "$DELIVERY/$f" ] || fail "缺少交付 $f"
done

# 交付副本與任務資料夾一致
cmp -s "$TASK/report.md" "$DELIVERY/2026-08-22 科技服務資訊週記.md" || fail "report.md 與交付副本不一致"
...
# 時間窗：所有正式來源發布日期需落在 2026-08-07 至 2026-08-22
formal_dates=$(printf '%s\n' "$formal_block" | sed -n 's/^- 發布日期：//p')
IFS='
'
for d in $formal_dates; do
  case "$d" in
    2026-08-0[7-9]|2026-08-1[0-9]|2026-08-2[0-2]) ;;
    *) fail "正式來源日期落在時間窗外：$d" ;;
  esac
done
IFS=$old_ifs
...
# 報告結構
grep -q '^# 2026-08-22 科技服務資訊週記$' "$REPORT" || fail "報告標題不符"
grep -q '^\*\*作者：Claude Code、Codex　報告日期：2026-08-22\*\*$' "$REPORT" || fail "報告 Meta 不符"
for anchor in summary highlights trends references notes disclaimer; do
...
assert_count 17 "$ref_count" "參考資料筆數"
...
python3 "$ROOT/.ai-global/skills/audit-tech-weekly-report/scripts/validate_report.py" \
  --report "$REPORT" \
  --source "$SOURCE" \
  --delivery-report "$DELIVERY/2026-08-22 科技服務資訊週記.md" \
  --delivery-source "$DELIVERY/source.md" \
  --start-date 2026-08-07 \
  --end-date 2026-08-22

echo "✅ weekly-tech-report-017 驗證通過（正式來源 17、淘汰 10、分類 12、引用 ref-1..17 一致）"
