...
|---|------|------|
| 1 | 確認日期範圍與分類 | 完成 |
| 2 | 蒐集來源並整理 source.md | 完成（四組研究代理平行蒐集後再補搜，候選 133 筆：正式 56 筆、淘汰 77 筆） |
...
- 報告日期：2026-09-26
- 時間範圍：2026-09-11 至 2026-09-26
- 使用工具：Claude Code（Fable 5.1）、Codex（GPT-5）；四組研究代理平行蒐集，主編收斂與撰稿，Codex 進行第二輪內容稽核
...
主編另補查 3 筆（Java 27、Anthropic 與 Akamai 算力合約、MIT 可吸收電池），並重抓金管會存款代幣原頁補齊被截斷的營業計畫書事項。收斂時以 `converge.py` 依指定網址順序把正式條目編號、剔除兩個撰稿欄位、淘汰條目編字母，並檢查摘要字數與時間窗。Codex 稽核補入 Apple Music Hall 官方來源，並將原 ref-49 移至淘汰區。最終候選 133 筆、正式 56 筆、淘汰 77 筆。
...
- **放射科 CAD 與 CADt 器材未獲 510(k) 部分豁免。** FDA 在聯邦公報刊登最終命令，正式駁回將放射科電腦輔助偵測／診斷與電腦輔助分診通知等四類器材部分豁免上市前通知的請願；駁回決定本身在 4 月 1 日作出，9 月 17 日刊登並生效 [[38. Federal Register]](#ref-38)。這表示請願涵蓋的器材類型仍須在上市前提交並取得 510(k) 許可，不代表每次軟體更新都必然重新送件。
...
- **非一手來源的使用。** GPT-6 Sol 與 Luna 以 iThome 為來源，因 openai.com 對抓取回 403、無法在原頁確認發布日 [[2. iThome]](#ref-2)；Docker Cloud Sandboxes 為 iThome 轉述 [[12. iThome]](#ref-12)；Akamai 合約與 IFR 統計為中央社編譯外電 [[30. 中央社]](#ref-30) [[24. 中央社]](#ref-24)；商用不動產數據為顧問公司報告的媒體轉述 [[46. 聯合新聞網]](#ref-46)；Netflix 案例為 NBC News 新聞報導 [[52. NBC News]](#ref-52)。
