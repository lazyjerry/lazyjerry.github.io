...
sh "$SKILL_SCRIPTS/validate-task-completion.sh" "$ROOT" weekly-tech-report-020

# 交付物完整性
[ -s "$DELIVERY/2026-09-13 科技服務資訊週記.md" ] || fail "缺少交付報告"
for f in source.md task_plan.md note.md references.md; do
  [ -s "$DELIVERY/$f" ] || fail "缺少交付 $f"
done

# 交付副本與任務資料夾一致
cmp -s "$TASK/report.md" "$DELIVERY/2026-09-13 科技服務資訊週記.md" || fail "report.md 與交付副本不一致"
...
# 時間窗：所有正式來源發布日期需落在 2026-08-30 至 2026-09-13
formal_dates=$(printf '%s\n' "$formal_block" | sed -n 's/^- 發布日期：//p')
IFS='
'
for d in $formal_dates; do
  case "$d" in
    2026-08-3[01]|2026-09-0[1-9]|2026-09-1[0-3]) ;;
    *) fail "正式來源日期落在時間窗外：$d" ;;
  esac
done
IFS=$old_ifs
...
# 報告結構
grep -q '^# 2026-09-13 科技服務資訊週記$' "$REPORT" || fail "報告標題不符"
grep -q '^\*\*作者：Claude Code、Codex　報告日期：2026-09-13\*\*$' "$REPORT" || fail "報告 Meta 不符"
...
assert_count 34 "$ref_count" "參考資料筆數"
...
python3 "$ROOT/.ai-global/skills/audit-tech-weekly-report/scripts/validate_report.py" \
  --report "$REPORT" \
  --source "$SOURCE" \
  --delivery-report "$DELIVERY/2026-09-13 科技服務資訊週記.md" \
  --delivery-source "$DELIVERY/source.md" \
  --start-date 2026-08-30 \
  --end-date 2026-09-13

echo "✅ weekly-tech-report-020 驗證通過（正式來源 34、淘汰 31、分類 12、引用 ref-1..34 一致）"
