...
sh "$SKILL_SCRIPTS/validate-task-completion.sh" "$ROOT" weekly-tech-report-018

# 交付物完整性
[ -s "$DELIVERY/2026-08-29 科技服務資訊週記.md" ] || fail "缺少交付報告"
for f in source.md task_plan.md note.md references.md; do
  [ -s "$DELIVERY/$f" ] || fail "缺少交付 $f"
done

# 交付副本與任務資料夾一致
cmp -s "$TASK/report.md" "$DELIVERY/2026-08-29 科技服務資訊週記.md" || fail "report.md 與交付副本不一致"
...
# 報告結構
grep -q '^# 2026-08-29 科技服務資訊週記$' "$REPORT" || fail "報告標題不符"
grep -q '^\*\*作者：Claude Code、Codex　報告日期：2026-08-29\*\*$' "$REPORT" || fail "報告 Meta 不符"
...
assert_count 21 "$ref_count" "參考資料筆數"
...
python3 "$ROOT/.ai-global/skills/audit-tech-weekly-report/scripts/validate_report.py" \
  --report "$REPORT" \
  --source "$SOURCE" \
  --delivery-report "$DELIVERY/2026-08-29 科技服務資訊週記.md" \
  --delivery-source "$DELIVERY/source.md" \
  --start-date 2026-08-14 \
  --end-date 2026-08-29

echo "✅ weekly-tech-report-018 驗證通過（正式來源 21、淘汰 15、分類 12、引用 ref-1..21 一致）"
